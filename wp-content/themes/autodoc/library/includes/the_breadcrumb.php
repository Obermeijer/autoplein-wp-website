...
function the_breadcrumb(){
	...
	global $post;
	echo '<ul class="breadcrumbs right"><li class="home"><a href="'.esc_url(home_url()).'">'.__('Home', 'PixTheme').'</a></li>';
		if(!is_front_page()){echo wp_kses_post($markup);}
...
function simple_breadcrumb_case($der_post){
	...
	if(is_single()){
		...
		if (is_attachment()){
			...
			else if($postType == 'portfolio')
			{
				...
				echo '<li>'.wp_kses_post($post_type->labels->singular_name.$markup).'</li>';
 				
				echo wp_kses_post($terms[0].$markup);
			}
			...
			else if($postType == 'news')
			{
				...
				$terms = explode('',$terms);
			 	$post_type = get_post_type_object(get_post_type()); 
				echo wp_kses_post($post_type->labels->singular_name.$markup);
	...
	echo "<li><a href='".esc_url(get_year_link($k_year))."'>".$k_year."</a></li>".$markup;
	...
	echo "<li><a href='".esc_url(get_year_link($k_year))."'>".$k_year."</a></li>".$markup;
	echo "<li><a href='".esc_url(get_month_link($k_year, $k_month))."'>".$k_month_display."</a></li>".$markup;
