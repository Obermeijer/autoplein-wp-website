	...
	class PixThemeFunctions {
		...
		function pixtheme_get_featured_image($args) {
  	 	...
  	 	function pixtheme_post_read_more() {
  	 		$btn_name = get_option('pixtheme_blog');
  	 		$name = (empty($btn_name['button_name'])) ? 'Read More':$btn_name['button_name'];
  	 		return '<a href="'.esc_url(get_permalink()).'" title="'.esc_attr(get_the_title()).'" class="readmore">'.$name.'</a>';
  	 	}
...
function pixtheme_get_option($slug,$_default = false){
	...
	if (isset($pix_options[$slug])){
		return wp_kses($pix_options[$slug],'default');
	}else{
		if ($_default)
