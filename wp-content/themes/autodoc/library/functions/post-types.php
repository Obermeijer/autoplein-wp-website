...
add_action( 'add_meta_boxes', 'pix_layout_side' );
function pix_layout_side() {
	add_meta_box(
		'pix_layout_side',
		'Page Layout',
		'pix_layout_side_content',
		null,
		'side', /* место размещения */
		'default'
	);
...
/* добавляем на страницу каталога новое поле контента для галереи*/
function pix_layout_side_content( $post ) {
	echo '<p><strong>Header Type</strong></p><p><select class="rwmb-select" name="header_type" />';
	$sel_ht = get_post_meta($post->ID, 'header_type', 1);
	echo '	<option value="" '.esc_attr(selected( $sel_ht, '' )).' >Default</option>
			<option value="pix-header1" '.esc_attr(selected( $sel_ht, 'pix-header1' )).' >Type 1</option>
			<option value="pix-header2" '.esc_attr(selected( $sel_ht, 'pix-header2' )).' >Type 2</option>
		</select></p>';
...
/* сохраняем изменения  */
add_action( 'save_post', 'pix_layout_side_save' );
function pix_layout_side_save( $post_id ) {
	if ( !empty($_POST['extra_fields_nonce']) && !wp_verify_nonce($_POST['extra_fields_nonce'], __FILE__) ) return false; // проверка
	if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE  ) return false; // выходим если это автосохранение
	if ( !current_user_can('edit_post', $post_id) ) return false; // выходим если юзер не имеет право редактировать запись
	...
	// Все ОК! Теперь, нужно сохранить/удалить данные
	$_POST['header_type'] = trim($_POST['header_type']); // чистим все данные от пробелов по краям
	$_POST['page_layout'] = trim($_POST['page_layout']); // чистим все данные от пробелов по краям
