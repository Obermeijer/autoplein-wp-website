...
jQuery(document).ready(function () {
	...
	jQuery("#pix_body_font, #pix_headings_font, #pix_menu_font").after('<div class="font-preview"></div>');
...
// image Uploader Functions ##############################################
	...
	jQuery(".pix_opts").parent().fadeIn(700);
	
	if (jQuery.cookie('promo-close-opt')){
		jQuery(".pix_opts").parent().prev().hide();
	}else{
		jQuery(".pix_opts").parent().prev().show();
	}
	
	jQuery('.opts-promo .promo-close').on('click',function(){

		jQuery.cookie('promo-close-opt',1);
		jQuery(".pix_opts").parent().prev().hide();
	});
