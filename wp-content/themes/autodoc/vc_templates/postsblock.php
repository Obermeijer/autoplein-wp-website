	...
	if ($wp_query->have_posts()): 
		while ($wp_query->have_posts()) : 							
			...
			if(!empty($pix_options['pix_blog_show_category']) && $pix_options['pix_blog_show_category'] == 1){
				...
				if($categories){
					...
					foreach($categories as $category) {
						$cat .= '<a href="'.esc_url(get_category_link( $category->term_id )).'" >'.$category->cat_name.'</a> ';
					}
					$cat .= '</span>';						
				}
			}
			...
			if($pix_options['pix_blog_show_date']){
				$date = '<span class="meta-i"> '.__('On: ', 'PixTheme').' <a href="#">'.get_the_time('M j, Y').'</a></span>';                    
			}
			$thumbnail = get_the_post_thumbnail( $post->ID ) != '' ? get_the_post_thumbnail( $post->ID, 'full' ) : '<img alt="'.__('No image', 'PixTheme').'"  src="'.esc_url(get_template_directory_uri()).'/images/noimage.jpg">';
...
$out .= '
				<li>
				  ...
				  <div class="carousel-item-content">
					...
					<a href="'.esc_url(get_the_permalink()).'" class="carousel-title">'.wp_kses_post(get_the_title()).'</a>
