			...
			<div class="container">
				<div class="row">
					<ul  id="filter"  class="portfolio-filter non-paginated">				
						<li><a href="#" class="current" data-filter="*">'. __('All Services' , 'PixTheme').'</a></li>';																		
...
switch ($pix_format){
	...
	case 'image':
		...
		$out .= '
								<div class="detail-item">
									<p>'.pixtheme_limit_words(get_the_excerpt(), 20).'</p>
									<a  href="'.esc_url($link).'" class="btn-icon-pt zoomPhoto"  rel="prettyPhoto[pp_gal_'.$post->ID.']"    >
	...
	case 'link':
		$out .= '
								<div class="detail-item">
									<a  href="'.esc_url(get_permalink(get_the_ID())).'"   data-toggle="modal" class="btn-icon-pt  btn-modal"  data-target="#myModal'.esc_attr(get_the_ID()).'" >
	...
	case 'video':
		...
		$out .= '							<a  href="'.esc_url($video_embed_code).'?width='.esc_attr($video_width).'&amp;height='. esc_attr($video_height).'" class="btn-icon-pt video-popab"   >
	...
	default:
		...
		$out .= '									
									...
									<a  href="'.esc_url($link).'" class="btn-icon-pt zoomPhoto" rel="prettyPhoto[pp_gal]"  >
										<i class="icon-picture icon-large"></i>
									</a>
								</div>';  
		break;
